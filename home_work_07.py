...
def add_contact(args, book: AddressBook):
    ...
    # Якщо запис не знайдено, створюємо новий і додаємо його до адресної книги.
    if record is None:
        record = Record(name)
        book.add_record(record)
        message = "Contact added."
    # Якщо вказаний номер телефону, додаємо його до контакту.
    if phone:
        record.add_phone(phone)
    # Повертаємо повідомлення про успішне додавання або оновлення контакту.
    return message
...
def change_contact(args, book):
    ...
    record = book.find(name)  # Пошук запису з вказаним ім'ям в адресній книзі.
    # Якщо знайдено відповідний запис, змінюємо номер телефону.
    if record:
...
def show_all(book):
    return "\n".join(
        [str(record) for record in book.data.values()])  # Повернення рядка з усією інформацією про всі контакти.


def parse_input(user_input):
    # Розбиваємо введений рядок на команду та аргументи.
    cmd, *args = user_input.split()
    # Перетворюємо команду у нижній регістр і видаляємо зайві пробіли.
    cmd = cmd.strip().lower()
